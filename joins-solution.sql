...
SELECT * FROM warehouse_product INNER JOIN warehouse ON warehouse_product.warehouse_id = warehouse.id WHERE warehouse_product.product_id = 5;

—-4. Which warehouses have diet pepsi?
SELECT * FROM warehouse_product INNER JOIN warehouse ON warehouse_product.warehouse_id = warehouse.id WHERE warehouse_product.product_id = 6;


—-5. Get the number of orders for each customer.
     NOTE: It is OK if those without orders are not included in results.

     SELECT first_name, last_name, COUNT(*) AS numberOfOrders
     FROM orders
